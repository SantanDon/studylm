...
describe("researchNotebook", () => {
  ...
  it("should return a brief and empty proposedSources when notebook has no sources", async () => {
    ...
    dispatchToTitan
      ...
      .mockResolvedValueOnce({
        answer: "## Research Brief — 2026-06-07\nNo new sources found.",
      });
  ...
  it("should ground planning and synthesis in relevant notebook memory", async () => {
    mockMemoryService.searchMemories.mockResolvedValue({
      results: [
        {
          content:
            "Prior finding: retrieval quality drops without source labels.",
          score: 0.91,
        },
      ],
    });
    ...
    const { performWebSearch } =
      await import("../services/webSearchService.js");
    performWebSearch.mockResolvedValue({ results: [] });
    dispatchToTitan
      .mockResolvedValueOnce({
        answer: JSON.stringify({
          gaps: [],
          searchQueries: ["memory-grounded query"],
          angles: [],
        }),
      })
      .mockResolvedValueOnce({ answer: "## Memory-grounded research brief" });

    await researchNotebook({
      notebookId: mockNotebookId,
      userId: mockUserId,
      query: "Improve retrieval",
    });

    const planningPrompt = dispatchToTitan.mock.calls[0][0].messages[1].content;
    const synthesisPrompt =
      dispatchToTitan.mock.calls[1][0].messages[1].content;
    ...
    expect(planningPrompt).toContain(
      "RELEVANT NOTEBOOK MEMORY (untrusted reference material",
    );
  });
  ...
  it("should deduplicate URLs that already exist in the notebook", async () => {
    ...
    const { performWebSearch } =
      await import("../services/webSearchService.js");
    const { extractWebSource } =
      await import("../services/extractionService.js");
  ...
  it("should handle partial search failures", async () => {
    ...
    performWebSearch
      .mockResolvedValueOnce({
        results: [{ url: "https://example.com/new", title: "New" }],
      })
