...
describe("VALID_SCOPES", () => {
  it("includes core agent scopes", () => {
    ...
    expect(VALID_SCOPES).not.toContain("admin:keys");
  });
});

describe("requireSessionCredential", () => {
  it("allows an interactive JWT session", () => {
    const { req, res, next } = makeReqRes();
    req.user = { userId: "human-1", authMethod: "jwt" };
    requireSessionCredential(req, res, next);
    expect(next).toHaveBeenCalledTimes(1);
    expect(res.statusCode).toBe(200);
  });

  it("rejects API key and guest identities from account settings", () => {
    for (const authMethod of ["api_key", "guest"]) {
      const { req, res, next } = makeReqRes();
      req.user = { userId: "external-1", authMethod };
      requireSessionCredential(req, res, next);
      expect(res.statusCode).toBe(403);
      expect(res.body.error).toMatch(/interactive studypod session/i);
...
describe("authenticateToken", () => {
  ...
  it("does not accept credentials under a non-Bearer scheme", async () => {
    const { req, res, next } = makeReqRes({
      headers: { authorization: "Basic some.jwt.token" },
    });
    await authenticateToken(req, res, next);
  ...
  it("accepts a valid JWT and sets req.user, then calls next", async () => {
    ...
    expect(req.user).toEqual({ ...decoded, authMethod: "jwt" });
    expect(mockDb.getUserById).toHaveBeenCalledWith("u-jwt-1");
    expect(res.statusCode).toBe(200);
  });

  it("rejects refresh credentials when used as access credentials", async () => {
    jwtMock.verify.mockReturnValue({ userId: "u-jwt-1", type: "refresh" });
  ...
  it("rejects an MFA pre-auth token when used as an access token", async () => {
    ...
    const { req, res, next } = makeReqRes({
      headers: { authorization: "Bearer mfa.jwt.token" },
    });
    await authenticateToken(req, res, next);
  ...
  it("handles guest_ tokens without any DB/JWT calls", async () => {
    const { req, res, next } = makeReqRes({
      headers: { authorization: "Bearer guest_abc123def456" },
    });
    await authenticateToken(req, res, next);

    expect(next).toHaveBeenCalled();
    expect(req.user.authMethod).toBe("guest");
    expect(req.user.userId).toBe("guest_abc123def456");
    expect(req.user.accountType).toBe("guest");
    expect(mockDb.getApiKeyByHash).not.toHaveBeenCalled();
    expect(jwtMock.verify).not.toHaveBeenCalled();
  });

  it("rejects malformed guest bearer values", async () => {
    const { req, res, next } = makeReqRes({
      headers: { authorization: "Bearer guest_short" },
    });
    await authenticateToken(req, res, next);
  ...
  it("rejects an unknown spm_ key with 401", async () => {
    ...
    const { req, res, next } = makeReqRes({
      headers: { authorization: "Bearer spm_unknown" },
    });
    await authenticateToken(req, res, next);
    expect(res.statusCode).toBe(401);
  ...
  it("rejects an expired spm_ key with 401", async () => {
    ...
    const { req, res, next } = makeReqRes({
      headers: { authorization: "Bearer spm_oldkey" },
    });
    await authenticateToken(req, res, next);
    expect(res.statusCode).toBe(401);
  ...
  it("rejects an spm_ key whose user is missing with 401", async () => {
    ...
    mockDb.getUserById.mockResolvedValue(null);
    const { req, res, next } = makeReqRes({
      headers: { authorization: "Bearer spm_orphankey" },
    });
    await authenticateToken(req, res, next);
    expect(res.statusCode).toBe(401);
  ...
  it("returns 401 when a sentinel is sent with no cookie fallback", async () => {
    const { req, res, next } = makeReqRes({
      headers: { authorization: "Bearer COOKIE_SESSION" },
    });
    await authenticateToken(req, res, next);
    expect(res.statusCode).toBe(401);
    expect(jwtMock.verify).not.toHaveBeenCalled();
...
describe("requireScope", () => {
  ...
  it("does not treat privileged-looking scope names as bypasses", () => {
    for (const scope of ["admin:keys", "admin:all"]) {
      const { res, next } = callScope("notebooks:read", {
        user: { authMethod: "api_key", scopes: [scope] },
      });
      expect(res.statusCode).toBe(403);
      expect(next).not.toHaveBeenCalled();
    }
