...
async function requireNotebookAccess(req, res, next) {
  try {
    ...
    logger.error("Notebook access check failed:", error);
    res.status(500).json({ error: "Access check failed" });
  }
}

async function getNotebookOrRecover(
  id,
  userId,
  ...
  const explicitlyDeleted = await dbHelpers.isNotebookExplicitlyDeleted(
    id,
    userId,
  );
  ...
  if (!notebook) {
    logger.info(
      `🛠️ JIT Recovery: Notebook ${id} missing. Attempting auto-provision...`,
    );
    try {
      await dbHelpers.createNotebook(
        id,
        userId,
        "Recovered Notebook",
        description,
  ...
  try {
    ...
    if (Array.isArray(req.user.restrictedNotebooks)) {
      notebooks = notebooks.filter((notebook) =>
        req.user.restrictedNotebooks.includes(notebook.id),
      );
    }

    notebooks.forEach((notebook) => {
      if (
        notebook.exampleQuestions &&
        typeof notebook.exampleQuestions === "string"
      ) {
        try {
...
router.post("/join", requireScope("notebooks:write"), async (req, res) => {
  try {
    ...
    const notebook = await dbHelpers.joinNotebookByCode(
      req.user.userId,
      code.toUpperCase(),
    );
    res.json({
...
router.put("/:id", requireScope("notebooks:write"), async (req, res) => {
  try {
    const { title, description, example_questions, generation_status, icon } =
      req.body;
    const updates = {};
    if (title !== undefined) updates.title = title;
    if (description !== undefined) updates.description = description;
    if (generation_status !== undefined)
      updates.generationStatus = generation_status;
    ...
    let notebook = await getNotebookOrRecover(
      req.params.id,
      req.user.userId,
      "Auto-provisioned from PUT",
    );
...
router.delete("/batch", requireScope("notebooks:write"), async (req, res) => {
  try {
    ...
    if (req.user.restrictedNotebooks) {
      const denied = ids.filter(
        (id) => !req.user.restrictedNotebooks.includes(id),
      );
      if (denied.length > 0) {
        return res.status(403).json({
          error: "API key is not authorized for one or more notebooks",
          denied,
        });
      }
    }
    ...
    res.json({
      message: `${result.changes} notebooks deleted successfully`,
...
router.delete("/:id", requireScope("notebooks:write"), async (req, res) => {
  try {
    const result = await dbHelpers.deleteNotebook(
      req.params.id,
      req.user.userId,
    );
...
router.post("/:id/notes", requireScope("notes:create"), async (req, res) => {
  try {
    let notebook = await getNotebookOrRecover(
      req.params.id,
      req.user.userId,
      "Automatically provisioned after system reset",
    );
    if (!notebook)
      return res.status(404).json({
        error: {
          code: "NOTEBOOK_NOT_FOUND",
          message: "Notebook not found and could not be recovered",
        },
      });
    ...
    if (
      req.user.authMethod === "api_key" ||
      (authorUser && authorUser.accountType === "agent")
    ) {
      await MemoryService.storeMemory(userId, req.params.id, content, {
        source: "agent_note",
    ...
    res
      .status(201)
      .json({ id, content, author_id, notebook_id: req.params.id });
  } catch (error) {
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (!note || note.notebookId !== req.params.id) {
...
router.put(
  "/:id/notes/:noteId",
  requireScope(["notes:write", "notes:create"]),
  requireNotebookAccess,
  async (req, res) => {
    try {
      const { content } = req.body;
      ...
      const existing = await dbHelpers.getNoteById(
        req.params.noteId,
        req.user.userId,
      );
      ...
      const result = await dbHelpers.updateNote(
        req.params.noteId,
        req.user.userId,
        content,
      );
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const result = await dbHelpers.deleteNote(
        req.params.noteId,
        req.user.userId,
      );
      if (result.changes === 0)
  ...
  async (req, res) => {
    try {
      ...
      if (!memory) {
        return res
          .status(500)
          .json({ error: "Pipeline failed to generate embedding" });
      }
...
router.post("/:id/sources", requireScope("sources:write"), async (req, res) => {
  try {
    ...
    if (processing_status && processing_status !== "pending") {
      try {
        await dbHelpers.updateSource(id, req.user.userId, {
          processing_status,
        });
      } catch (e) {
    ...
    const createdSources = await dbHelpers.getSourcesByNotebookId(
      req.params.id,
      req.user.userId,
    );
    const createdSource = createdSources.find((source) => source.id === id);
    res.status(201).json(
      createdSource || {
        id,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (fileContent) {
        const { parseTwitterBookmarksExport } =
          await import("../services/tweetExtractionService.js");
      ...
      if (tweetInputs.length === 0) {
        return res
          .status(400)
          .json({ error: "No tweet URLs or bookmarks archive file provided" });
      }

      // Process deep dive sequentially
      const result = await deepDiveBookmarks(
        tweetInputs,
        req.params.id,
        req.user.userId,
      );
      ...
      res.status(500).json({ error: "Failed to process tweet bookmarks" });
    }
  },
);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const requestedProcessingStatus =
        updates.processing_status || updates.processingStatus;
      const isIndexingTransition = ["processing", "indexing"].includes(
        requestedProcessingStatus,
      );
      if (isIndexingTransition) {
        ...
        const existingSource = sources.find(
          (source) => source.id === req.params.sourceId,
        );
        const content =
        ...
        if (shouldSkipSemanticIndexing(content.length)) {
          ...
          updates.metadata = buildKeywordOnlySourceMetadata(
            metadata,
            content.length,
          );
        }
      }
      const result = await dbHelpers.updateSource(
        req.params.sourceId,
        req.user.userId,
        updates,
      );
      ...
      if (result.changes === 0) {
        logger.info(
          `🛠️ PUT /sources/:sourceId: Source missing, auto-provisioning...`,
        );
        try {
          ...
          if (updates.processing_status) {
            await dbHelpers.updateSource(req.params.sourceId, req.user.userId, {
          ...
          return res
            .status(404)
            .json({ error: "Source not found and could not be recovered" });
        }
      }
...
router.delete(
  ...
  async (req, res) => {
    try {
      const result = await dbHelpers.deleteSource(
        req.params.sourceId,
        req.user.userId,
      );
      if (result.changes === 0) {
 ...
 */
router.post(
  "/:id/sources/:sourceId/generate-hooks",
  requireScope("notes:create"),
  async (req, res) => {
    try {
      ...
      if (!source) {
        return res
          .status(404)
          .json({ error: "Source not found or access denied" });
      }

      if (!source.content || source.content.length < 50) {
        return res.status(400).json({
          error:
            "Source content is too thin for high-quality signal generation.",
        });
      }

      // 2. Generate Hooks
      logger.info(
        `🧬 [SOVEREIGN SIGNAL] Generating hooks for source: ${source.title}`,
      );
      ...
      try {
        ...
        logger.warn(
          `🧬 [SOVEREIGN SIGNAL] Failed to persist signal hooks to queue: ${err.message}`,
        );
      }
      ...
      res.json({
        ...
        noteId,
        message:
          "Sovereign Signal generated, saved as note, and staged to Signal Queue.",
      });
    } catch (error) {
      logger.error("Signal generation error:", error);
      res.status(500).json({ error: "Sovereign Signal failed" });
    }
  },
);
...
router.get(
  "/:id/messages",
  requireScope(["chat:all", "chat:readonly"]),
  async (req, res) => {
    try {
      let notebook = await getNotebookOrRecover(req.params.id, req.user.userId);
      if (!notebook)
      ...
      const messages = await dbHelpers.getChatMessagesByNotebookId(
        req.params.id,
        req.user.userId,
      );
      res.json(messages);
 ...
 */
router.delete("/:id/messages", requireScope("chat:all"), async (req, res) => {
  try {
    let notebook = await getNotebookOrRecover(req.params.id, req.user.userId);
    if (!notebook)
      return res.status(404).json({
    ...
    await dbHelpers.deleteChatMessagesByNotebookId(
      req.params.id,
      req.user.userId,
    );
  ...
  try {
    ...
    res.json({
      ...
      sources: sources.map((s) => {
        ...
        const contentStatus = ["pending", "uploading"].includes(
          processingStatus,
        )
          ? "pending"
        ...
        return {
          ...
          transcriptStatus:
            trust.transcriptStatus || (s.type === "youtube" ? "unknown" : null),
        };
      }),
      notes: notes.map((n) => ({
        id: n.id,
      ...
      agentCapabilities: {
        ...
        youtube: {
          ingest:
            "POST /api/youtube/ingest with { notebookId, url, language? }",
          providers: "GET /api/youtube/providers",
          transcript:
            "GET /api/youtube/youtube-transcript?url=<youtube-url>&language=<language-code>",
          sourceTimeline: `GET /api/notebooks/${req.params.id}/sources/:sourceId/content`,
          qualityContract: {
            transcriptStatus: "full | partial | metadata_only",
            videoAvailability:
              "available | available_no_transcript | private | restricted | region_blocked | unavailable | unknown",
            timingQuality: "provider | mixed | inferred | none",
            seekableCitations:
              "Use only when source.providerCapabilities.seekableCitations is true.",
          },
          recommendedWorkflow:
            "Use ingest for a one-call notebook source. Inspect source transcriptQuality before chat, and use sourceIds to scope evidence-sensitive questions.",
        },
      },
      agentReady: true,
    });
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const result = await MasticationService.immerseInSource(
        notebookId,
        userId,
        sourceId,
        agentId,
      ...
      if (!result) {
        return res.status(404).json({
          error: "Source not found or source has no readable content",
        });
      }

      res.json({
        message: "Source immersion completed. Grounded margin notes are ready.",
        result,
      });
    } catch (error) {
...
router.post("/:id/chat", requireScope("chat:all"), async (req, res) => {
  try {
    ...
    if (
      requestedSourceIds.length > 0 &&
      scopedSources.length !== requestedSourceIds.length
    ) {
      return res.status(400).json({
    ...
    if (
      normalizedMsg.includes("closed-loop synthesis") ||
      normalizedMsg.includes("synthesize bookmarks and align")
    ) {
      const goals = await dbHelpers.getGoalsByNotebookId(notebookId, {
      ...
      if (!goals || goals.length === 0) {
        ...
        if (readySources.length === 0) {
          ...
          const brokerResult = await brokerResearchGoals(
            notebookId,
            userId,
            sourceIds,
          );
      ...
      normalizedMsg.includes("mine bookmarks") ||
      normalizedMsg.includes("mine replies")
    ) {
      ...
      if (tweetSources.length === 0) {
        ...
        const deepDiveResult = await deepDiveBookmarks(
          urls,
          notebookId,
          userId,
        );
      ...
      normalizedMsg.includes("extract github repositories") ||
      normalizedMsg.includes("extract repositories") ||
      normalizedMsg.includes("extract github")
    ) {
      const gitHubRegex =
      ...
      if (repos.length === 0) {
        ...
        let tasksCreated = 0;
        const existingTasks = await dbHelpers.getTasksByNotebookId(
          notebookId,
          userId,
        );
        const existingTaskContents = (existingTasks || []).map((t) =>
        ...
        for (const repo of repos) {
          ...
          if (
            !existingTaskContents.some((c) => c.includes(repo.toLowerCase()))
          ) {
            await dbHelpers.createTask(
              userId,
              notebookId,
              taskContent,
              "agent",
      ...
      normalizedMsg.includes("draft social media updates") ||
      normalizedMsg.includes("stage to signal queue") ||
      normalizedMsg.includes("draft updates")
    ) {
      const readySources = sources.filter(
        (s) =>
          s.processingStatus === "completed" ||
        ...
        for (const src of readySources) {
          ...
          if (content.length > 50) {
            try {
              ...
              logger.warn(
                `Failed to auto-generate hooks during chat action: ${err.message}`,
              );
            }
          }
        }
    ...
    if (interceptedResponse) {
      ...
      await WebhookDispatcher.recordActivityAndNotify(
        notebookId,
        userId,
        callerIsAgent ? getActorName(req) : "human",
        "chat.message",
    ...
    if (isTransientDatabaseError(error)) {
      return res.status(503).json({
        error:
          "The notebook database is temporarily unavailable. Please retry shortly.",
...
router.post("/:id/research", requireScope("chat:all"), async (req, res) => {
  try {
    ...
    const result = await researchNotebook({
      notebookId: id,
      userId,
      query,
      depth,
    ...
    await dbHelpers.createActivityLog(
      ...
      getActorName(req),
      "research_further",
      `Research further: ${query || "explore gaps"}`,
    );
...
router.get(
  ...
  async (req, res) => {
    try {
      const activities = await dbHelpers.getActivityLogsByNotebookId(
        req.params.id,
      );
      res.json({ activities });
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        ...
        transcriptSegments:
          source.type === "youtube" &&
          Array.isArray(metadata.transcriptSegments)
            ? metadata.transcriptSegments
            : [],
        transcriptProvider:
          metadata.transcriptProvider || metadata.extractedBy || null,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const due_by = req.body.due_by || null;
      if (!instruction || instruction.length > 4000) {
        return res.status(400).json({
          error: "Task instruction must contain 1-4000 characters",
        });
      }
      if (
        !["human", "agent"].includes(assignee) ||
        !["low", "medium", "high"].includes(priority)
      ) {
        return res
          .status(400)
          .json({ error: "Invalid task assignee or priority" });
      }
      if (due_by && Number.isNaN(new Date(due_by).getTime())) {
        return res.status(400).json({ error: "due_by must be a valid date" });
      }
      ...
      await dbHelpers.createActivityLog(
        ...
        getActorName(req),
        "created_task",
        `Task assigned to ${assignee || "human"}: ${instruction.substring(0, 50)}...`,
      );
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const result = req.body.result == null ? null : String(req.body.result);
      if (!["pending", "completed"].includes(status)) {
        return res
          .status(400)
          .json({ error: "Task status must be pending or completed" });
      }
      if (result && result.length > 100_000) {
        return res
          .status(400)
          .json({ error: "Task result must be 100000 characters or fewer" });
      }
      const task = await dbHelpers.getTaskById(
        req.params.taskId,
        req.user.userId,
        req.params.id,
      );
      ...
      await dbHelpers.createActivityLog(
        ...
        getActorName(req),
        "updated_task",
        `Task ${req.params.taskId} marked as ${status}`,
      );

      res.json({ message: "Task updated", task: { ...task, ...updates } });
    } catch (error) {
...
router.get(
  ...
  async (req, res) => {
    try {
      const entries = await dbHelpers.getScratchpadByNotebookId(
        req.params.id,
        req.user.userId,
      );
  ...
  async (req, res) => {
    try {
      ...
      const expiresAt = ttl_hours
        ...
        : null;
      const entry = await dbHelpers.createScratchpadEntry(
        req.params.id,
        req.user.userId,
        content,
        expiresAt,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!entry)
        return res.status(404).json({ error: "Scratchpad entry not found" });
      ...
      await dbHelpers.createActivityLog(
        ...
        getActorName(req),
        "promoted_scratchpad",
        "Promoted a scratchpad entry to a persistent note",
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const requestedEvents = Array.isArray(events)
        ? [...new Set(events.map(String))]
        : null;
      const supportedEvents = new Set(["*", ...WebhookDispatcher.EVENT_TYPES]);
      if (
        !requestedEvents ||
        requestedEvents.length > 50 ||
        requestedEvents.some((event) => !supportedEvents.has(event))
      ) {
        return res.status(400).json({
          error: "events must be an array of supported webhook event names",
        });
      }

      let safeUrl;
      try {
        safeUrl = await assertSafeExternalHttpsUrl(url);
      } catch (error) {
        return res.status(400).json({ error: error.message });
      }

      const hook = await dbHelpers.createWebhook(
        req.params.id,
        req.user.userId,
        safeUrl,
        JSON.stringify(requestedEvents),
      );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (mission === "start") {
        await agentPulse.startMission(
          userId,
          notebookId,
          thought || "Unnamed mission",
        );
        return res.json({ message: "Mission started" });
      }
      if (mission === "end") {
        await agentPulse.endMission(userId, notebookId, thought);
...
router.get(
  "/:id/research-goals",
  requireScope("notebooks:read"),
  requireNotebookAccess,
  async (req, res) => {
    try {
      const includeArchived = req.query.includeArchived === "true";
      const parentGoalId = req.query.parentGoalId || null;
      const goals = await dbHelpers.getGoalsByNotebookId(req.params.id, {
        includeArchived,
        parentGoalId,
      });
      res.json({ goals });
  ...
  async (req, res) => {
    try {
      ...
      if (!goal || typeof goal !== "string" || goal.length < 3) {
        return res
          .status(400)
          .json({ error: "goal is required (min 3 chars)" });
      }
 ...
 */
router.post(
  "/:id/research-goals/:goalId/tasks/:taskId",
  requireScope("notes:create"),
  requireNotebookAccess,
  async (req, res) => {
    try {
      ...
      if (!task || task.notebookId !== req.params.id) {
        return res
          .status(404)
          .json({ error: "Task not found in this notebook" });
      }
      const updated = await dbHelpers.linkTaskToGoal(
        req.params.goalId,
        req.params.taskId,
      );
      res.json({ goal: updated });
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      const updated = await dbHelpers.unlinkTaskFromGoal(
        req.params.goalId,
        req.params.taskId,
      );
      res.json({ goal: updated });
 ...
 */
router.post(
  "/:id/research-goals/:goalId/artifacts",
  requireScope("notes:create"),
  requireNotebookAccess,
  async (req, res) => {
    try {
      ...
      const updated = await dbHelpers.linkArtifactToGoal(
        req.params.goalId,
        artifactId,
      );
      res.json({ goal: updated });
  ...
  async (req, res) => {
    try {
      ...
      if (!existing || existing.notebookId !== req.params.id) {
        return res
          .status(404)
          .json({ error: "Research goal not found or unauthorized" });
      }
      ...
      await dbHelpers.createActivityLog(
        ...
        `Deleted research goal "${existing.title}"`,
      );
 ...
 */
router.get(
  "/:id/sources/:sourceId/suggested-goals",
  requireScope("notebooks:read"),
  requireNotebookAccess,
  async (req, res) => {
    try {
      const { getOrComputeSuggestedGoals } =
        await import("../services/suggestedGoalsService.js");
      const force = req.query.force === "true";
      const { suggestions, computed, rateLimited, retryIn } =
        await getOrComputeSuggestedGoals(
          req.params.id,
          req.user.userId,
          req.params.sourceId,
          { force },
 ...
 */
router.post(
  "/:id/sources/:sourceId/suggested-goals/:suggestionId/accept",
  requireScope("notes:create"),
  requireNotebookAccess,
  async (req, res) => {
    try {
      const suggestions = await dbHelpers.getSignalQueueSuggestionsBySource(
        req.params.sourceId,
      );
      const suggestion = suggestions.find(
        (s) => s.id === req.params.suggestionId,
      );
      if (!suggestion)
      ...
      const goalId = await dbHelpers.createGoal({
        ...
        sourceId: req.params.sourceId,
        sourceChunkId:
          Array.isArray(suggestion.sourceChunkIndices) &&
          suggestion.sourceChunkIndices[0] != null
            ? String(suggestion.sourceChunkIndices[0])
            : null,
        priority: "medium",
        status: "active",
      });
      ...
      await dbHelpers.createActivityLog(
        ...
        `Accepted suggested goal "${goal.title}"`,
      );
