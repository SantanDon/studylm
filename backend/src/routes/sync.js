...
router.use(authenticateToken, requireSessionCredential);
...
router.post("/upload", async (req, res) => {
  try {
    ...
    const existing = await db
      .select({ version: schema.sync_data.version })
      .from(schema.sync_data)
      .where(
        and(eq(schema.sync_data.id, id), eq(schema.sync_data.userId, userId)),
      )
      .limit(1);
    ...
    if (existing && existing.length > 0) {
      ...
      // Insert new record
      await db.insert(schema.sync_data).values({
        id,
        userId,
        type,
        encryptedData,
...
router.post("/batch-upload", async (req, res) => {
  try {
    ...
    if (items.length > 100) {
      return res
        .status(413)
        .json({ error: "A sync batch can contain at most 100 items" });
    }
      ...
      for (const item of items) {
        ...
        try {
          ...
          if (existing && existing.length > 0) {
            ...
            await tx.insert(schema.sync_data).values({
              id,
              userId,
              type,
              encryptedData,
          ...
          logger.warn("Sync batch item failed:", error.message);
          results.push({ id, success: false, error: "Sync item failed" });
        }
      }
    });
