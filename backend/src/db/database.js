...
import { eq, sql, and, or, desc, asc, inArray, lt } from "drizzle-orm";
...
if (!url || !authToken) {
  if (isVercel) {
    throw new Error(
      "❌ [SECURITY] TURSO_DATABASE_URL and TURSO_AUTH_TOKEN must be set in Vercel environment.",
    );
  }
  // Local fallback logic if needed, but the secret MUST be removed
  logger.warn(
    "Database credentials missing. Falling back to local configuration check.",
  );
}
  ...
  if (!dbInstance) {
    try {
      ...
      if (
        ...
        logger.info(
          "Using Web client (@libsql/client/web) for HTTP/libsql protocol",
        );
        createClientFn = createWebClient;
      } else {
        logger.info(
          "Using standard client (@libsql/client) for local file protocol",
        );
...
const databaseInitTimeoutMs = Number(
  process.env.DATABASE_INIT_TIMEOUT_MS || 30_000,
);
...
async function runDatabaseInitialization() {
  ...
  if (url?.startsWith("file:")) {
    ...
    if (isBrandNewDatabase) {
      ...
      logger.info(
        "Existing local schema detected; skipping baseline migration and applying compatibility checks.",
      );
    }
  }
  ...
  try {
    ...
    for (const [col, decl] of userAlters) {
      try {
        await db.run(sql.raw(`ALTER TABLE users ADD COLUMN ${col} ${decl}`));
      } catch (e) {
        logger.debug(`Schema migration (users.${col}): ${e.message}`);
      }
    }
    ...
    try {
      await db.run(
        sql`ALTER TABLE api_keys ADD COLUMN scopes text DEFAULT '["notebooks:read","notes:create","chat:all"]'`,
      );
    } catch (e) {
      logger.debug(`Schema migration (scopes): ${e.message}`);
    }
    try {
      await db.run(sql`ALTER TABLE api_keys ADD COLUMN notebook_ids text`);
    } catch (e) {
      logger.debug(`Schema migration (notebook_ids): ${e.message}`);
    }
    try {
      await db.run(sql`ALTER TABLE api_keys ADD COLUMN expires_at integer`);
    } catch (e) {
      logger.debug(`Schema migration (expires_at): ${e.message}`);
    }
    try {
      await db.run(
        sql`ALTER TABLE api_keys ADD COLUMN rate_limit integer DEFAULT 0`,
      );
    } catch (e) {
      logger.debug(`Schema migration (rate_limit): ${e.message}`);
    }
    await db.run(sql`CREATE TABLE IF NOT EXISTS "oauth_clients" (
    ...
    await db.run(sql`CREATE TABLE IF NOT EXISTS "oauth_authorization_codes" (
    ...
    await db.run(sql`CREATE TABLE IF NOT EXISTS "oauth_refresh_tokens" (
      "token_hash" text PRIMARY KEY NOT NULL,
      "client_id" text NOT NULL REFERENCES oauth_clients(client_id) ON DELETE CASCADE,
      "user_id" text NOT NULL REFERENCES users(id) ON DELETE CASCADE,
      "scopes" text NOT NULL,
      "access_key_id" text REFERENCES api_keys(id) ON DELETE SET NULL,
      "expires_at" integer NOT NULL,
    ...
    await db.run(
      sql`CREATE INDEX IF NOT EXISTS documents_notebook_idx ON documents(notebook_id, updated_at DESC)`,
    );
    await db.run(
      sql`CREATE INDEX IF NOT EXISTS document_versions_document_idx ON document_versions(document_id, version DESC)`,
    );
    ...
    for (const [col, decl] of signalQueueAlters) {
      try {
        await db.run(
          sql.raw(`ALTER TABLE signal_queue ADD COLUMN ${col} ${decl}`),
        );
      } catch (e) {
        logger.debug(`Schema migration (signal_queue.${col}): ${e.message}`);
      }
    }
    ...
    for (const [col, decl] of goalAlters) {
      try {
        await db.run(
          sql.raw(`ALTER TABLE research_goals ADD COLUMN ${col} ${decl}`),
        );
      } catch (e) {
        logger.debug(`Schema migration (research_goals.${col}): ${e.message}`);
      }
    }
    ...
    logger.warn(
      "Schema fallback creation skipped (tables may already exist via Drizzle):",
      err.message,
    );
  }
...
export const dbHelpers = {
  ...
  async getUserByDisplayName(displayName) {
    ...
    const result = await db
      .select()
      .from(schema.users)
      .where(eq(schema.users.displayName, displayName))
      .limit(1);
    return result[0];
  },
  ...
  async storeRecoveryHash(userId, hash) {
    ...
    return await db
      .update(schema.users)
      .set({ recoveryHash: hash, updatedAt: new Date() })
  ...
  async markRecoveryTokenUsed(id) {
    const db = await getDatabase();
    return await db
      .update(schema.recoveryTokens)
      .set({ used: true })
      .where(eq(schema.recoveryTokens.id, id));
  },
  ...
  async getUserByRecoveryHash(hash) {
    ...
    const result = await db
      .select()
      .from(schema.users)
      .where(eq(schema.users.recoveryHash, hash))
      .limit(1);
    return result[0];
  },
  ...
  async getNotebookById(id, userId) {
    ...
    const result = await db
      ...
      .leftJoin(
        schema.notebookMembers,
        eq(schema.notebooks.id, schema.notebookMembers.notebookId),
      )
      .where(
        and(
  ...
  async getNotebooksByUserId(userId) {
    ...
    const countsMap = Object.fromEntries(
      countsResult.map((c) => [c.notebookId, Number(c.count)]),
    );
  ...
  async getNotebooksWithDeepContext(userId) {
    ...
    const [allSources, allNotes] = await Promise.all([
      ...
      db
        ...
        .from(schema.notes)
        .where(inArray(schema.notes.notebookId, ids)),
    ]);
  ...
  async updateNotebook(id, userId, updates) {
    ...
    return await db
      ...
      .where(
        and(eq(schema.notebooks.id, id), eq(schema.notebooks.userId, userId)),
      );
  },
  ...
  async deleteNotebook(id, userId) {
    ...
    if (notebook) {
      ...
      try {
        ...
        logger.debug(
          `deletedNotebooks record insertion fail/ignore: ${err.message}`,
        );
      }
    ...
    const membership = await db
      .delete(schema.notebookMembers)
      .where(
        and(
          eq(schema.notebookMembers.notebookId, id),
          eq(schema.notebookMembers.userId, userId),
        ),
      );
    ...
    if (result.rowsAffected > 0) {
      for (const id of ids) {
        try {
          ...
          logger.debug(
            `deletedNotebooks batch insertion fail/ignore: ${err.message}`,
          );
        }
      }
    }
  ...
  async createChatMessage(
    ...
    const groundedSourcesStr = groundedSources
      ? typeof groundedSources === "string"
        ? groundedSources
    ...
    const results = {
      notebooks: await db
        ...
        .where(
          and(
            eq(schema.notebooks.userId, userId),
            sql`${schema.notebooks.title} LIKE ${cleanQuery}`,
          ),
        ),
      sources: await db
        .select()
        ...
        .where(
          and(
            ...
            sql`${schema.sources.title} LIKE ${cleanQuery} OR ${schema.sources.content} LIKE ${cleanQuery}`,
          ),
        ),
      notes: await db
        .select()
        ...
        .where(
          and(
            ...
            sql`${schema.notes.content} LIKE ${cleanQuery}`,
          ),
        ),
    };
  ...
  async createTag(userId, notebookId, name, color = "#6366f1") {
    ...
    return await db
      .insert(schema.tags)
      .values({ id, userId, notebookId, name, color });
  },
  ...
  async getTasksByNotebookId(notebookId, userId = null) {
    const db = await getDatabase();
    const filters = [eq(schema.tasks.notebookId, notebookId)];
    if (userId) filters.push(eq(schema.tasks.userId, userId));
    return await db
      .select()
      .from(schema.tasks)
      .where(and(...filters))
  ...
  async updateTask(id, updates, userId = null, notebookId = null) {
    const db = await getDatabase();
    const filters = [eq(schema.tasks.id, id)];
    if (userId) filters.push(eq(schema.tasks.userId, userId));
    if (notebookId) filters.push(eq(schema.tasks.notebookId, notebookId));
    return await db
      .update(schema.tasks)
      .set({ ...updates, updatedAt: new Date() })
      .where(and(...filters));
  ...
  async getTaskById(id, userId = null, notebookId = null) {
    const db = await getDatabase();
    const filters = [eq(schema.tasks.id, id)];
    if (userId) filters.push(eq(schema.tasks.userId, userId));
    if (notebookId) filters.push(eq(schema.tasks.notebookId, notebookId));
  ...
  async getGoalsByNotebookId(
    notebookId,
    { includeArchived = false, parentGoalId = null } = {},
  ) {
    const db = await getDatabase();
    ...
    const rows = await db
      .select()
      .from(schema.researchGoals)
      .where(and(...conds))
      .orderBy(
        desc(schema.researchGoals.priority),
        desc(schema.researchGoals.lastActivityAt),
        desc(schema.researchGoals.createdAt),
      );
    return rows.map((r) => ({
      ...r,
      linkedTaskIds: r.linkedTaskIds ? JSON.parse(r.linkedTaskIds) : [],
      linkedArtifactIds: r.linkedArtifactIds
        ? JSON.parse(r.linkedArtifactIds)
        : [],
    }));
  },
  ...
  async getGoalById(id) {
    ...
    const result = await db
      .select()
      .from(schema.researchGoals)
      .where(eq(schema.researchGoals.id, id))
      .limit(1);
  ...
  async getGoalChildren(parentGoalId) {
    ...
    return await db
      .select()
      .from(schema.researchGoals)
      .where(eq(schema.researchGoals.parentGoalId, parentGoalId));
  },
  ...
  async linkTaskToGoal(goalId, taskId) {
    ...
    await db
      .update(schema.researchGoals)
      .set({
        linkedTaskIds: JSON.stringify(ids),
  ...
  async linkArtifactToGoal(goalId, artifactId) {
    ...
    const ids = Array.from(
      new Set([...(goal.linkedArtifactIds || []), artifactId]),
    );
    await db
      .update(schema.researchGoals)
      .set({
        linkedArtifactIds: JSON.stringify(ids),
        lastActivityAt: new Date(),
        updatedAt: new Date(),
      })
      .where(eq(schema.researchGoals.id, goalId));
  ...
  async computeGoalProgress(goalId) {
    ...
    if (taskIds.length === 0)
      return { ...goal, progressPct: 0, derivedFrom: "no-tasks" };
    const db = await getDatabase();
    const placeholders = taskIds.map(() => "?").join(",");
    const rows = await db.all(
      sql.raw(`SELECT status FROM tasks WHERE id IN (${placeholders})`),
      taskIds,
    );
    if (rows.length === 0)
      return { ...goal, progressPct: 0, derivedFrom: "no-tasks" };
  ...
  async upsertSignalQueueSuggestion({
    id,
    sourceId,
    notebookId,
    userId,
    ...
    const existing = await db
      .select()
      .from(schema.signalQueueSuggestions)
      .where(
        and(
          eq(schema.signalQueueSuggestions.sourceId, sourceId),
          eq(schema.signalQueueSuggestions.title, title),
        ),
      )
      .limit(1);
    ...
    if (existing[0]) {
      await db
        .update(schema.signalQueueSuggestions)
        .set({
          rationale,
          sourceChunkIndices: indices,
          confidence,
          status,
          updatedAt: new Date(),
        })
  ...
  async getSignalQueueSuggestionsBySource(sourceId) {
    ...
    return await db
      ...
      .orderBy(
        desc(schema.signalQueueSuggestions.confidence),
        desc(schema.signalQueueSuggestions.createdAt),
      );
  },
  ...
  async createActivityLog(
    ...
    return await db
      ...
      .values({ id, notebookId, userId, actor, actionType, contentPreview });
  },
  ...
  async getActivityLogsByNotebookId(notebookId) {
    ...
    return await db
      ...
      .where(eq(schema.activityLog.notebookId, notebookId))
      .orderBy(desc(schema.activityLog.createdAt));
  },

  // Sovereign Bridge - Scratchpad
  async createScratchpadEntry(
    notebookId,
    userId,
    content,
    ttlExpiresAt = null,
    ...
    return await db.insert(schema.scratchpad).values({
      id,
      notebookId,
      userId,
      content,
  ...
  async deleteScratchpadEntry(id) {
    const db = await getDatabase();
    return await db
      .delete(schema.scratchpad)
      .where(eq(schema.scratchpad.id, id));
  },
  ...
  async createWebhook(notebookId, userId, url, eventsJson) {
    ...
    return await db
      ...
      .values({ id, notebookId, userId, url, eventsJson });
  },
  ...
  async getWebhooksByNotebookId(notebookId) {
    ...
    return await db
      ...
      .where(eq(schema.webhooks.notebookId, notebookId));
  },
  ...
  async acceptSource(sourceId, userId) {
    ...
    return await db
      ...
      .where(
        and(eq(schema.sources.id, sourceId), eq(schema.sources.userId, userId)),
      );
  },
  ...
  async getMemoriesByNotebook(notebookId, userId = null) {
    const db = await getDatabase();
    return await db
      .select()
      ...
      .where(
        userId
          ? and(
              eq(schema.memories.notebookId, notebookId),
  ...
  async createOAuthClient(clientId, clientName, redirectUris, grantTypes) {
    const db = await getDatabase();
    ...
    return { clientId, clientName, redirectUris, grantTypes };
  },
  ...
  async createOAuthAuthorizationCode({
    ...
    const db = await getDatabase();
    return db.insert(schema.oauthAuthorizationCodes).values({
      codeHash,
      clientId,
      userId,
      redirectUri,
  ...
  async getOAuthAuthorizationCode(codeHash) {
    const db = await getDatabase();
    const rows = await db
      .select()
  ...
  async consumeOAuthAuthorizationCode(codeHash) {
    ...
    return updated.rowsAffected > 0 ? record : null;
  },
  ...
  async createOAuthRefreshToken({
    ...
    const db = await getDatabase();
    return db.insert(schema.oauthRefreshTokens).values({
      tokenHash,
      clientId,
      userId,
      scopes: JSON.stringify(scopes),
  ...
  async consumeOAuthRefreshToken(tokenHash) {
    const db = await getDatabase();
    const rows = await db
      .select()
      .from(schema.oauthRefreshTokens)
    ...
    const updated = await db
      ...
      .where(
        and(
          ...
          eq(schema.oauthRefreshTokens.revoked, false),
        ),
      );
    return updated.rowsAffected > 0 ? record : null;
  },
  ...
  async listOAuthConnections(userId) {
    ...
    return db
      .select({
        ...
        scopes: schema.oauthRefreshTokens.scopes,
        accessKeyId: schema.oauthRefreshTokens.accessKeyId,
        expiresAt: schema.oauthRefreshTokens.expiresAt,
        createdAt: schema.oauthRefreshTokens.createdAt,
        lastUsedAt: schema.apiKeys.lastUsedAt,
      })
      .from(schema.oauthRefreshTokens)
      .innerJoin(
        schema.oauthClients,
        eq(schema.oauthClients.clientId, schema.oauthRefreshTokens.clientId),
      )
      .leftJoin(
        schema.apiKeys,
        eq(schema.apiKeys.id, schema.oauthRefreshTokens.accessKeyId),
      )
      .where(
        and(
      ...
      .orderBy(desc(schema.oauthRefreshTokens.createdAt));
  },

  async revokeOAuthRefreshToken(tokenHash) {
    const db = await getDatabase();
    return db
      .update(schema.oauthRefreshTokens)
      .set({ revoked: true })
  ...
  async revokeOAuthConnection(userId, clientId) {
    const db = await getDatabase();
    const rows = await db
      .select({ accessKeyId: schema.oauthRefreshTokens.accessKeyId })
      .from(schema.oauthRefreshTokens)
      .where(
        and(
          eq(schema.oauthRefreshTokens.userId, userId),
    ...
    const accessKeyIds = [
      ...new Set(rows.map((row) => row.accessKeyId).filter(Boolean)),
    ];
    for (const accessKeyId of accessKeyIds) {
      await db
        ...
        .where(
          and(
            eq(schema.apiKeys.id, accessKeyId),
            eq(schema.apiKeys.userId, userId),
          ),
        );
    }
    return {
      revoked: updateResult.rowsAffected || 0,
      accessKeysRevoked: accessKeyIds.length,
    };
  },
  ...
  async createAgentMission(
    ...
    const db = await getDatabase();
    return await db.insert(schema.agentMissions).values({
      id,
      userId,
      notebookId,
      goal,
  ...
  async getAgentMissionsByNotebookId(notebookId, userId = null) {
    const db = await getDatabase();
    const conditions = [eq(schema.agentMissions.notebookId, notebookId)];
    if (userId) conditions.push(eq(schema.agentMissions.userId, userId));
    return await db
      .select()
      .from(schema.agentMissions)
  ...
  async claimAgentMission(id, staleAfterMs = 10 * 60 * 1000) {
    const db = await getDatabase();
    const staleBefore = new Date(Date.now() - staleAfterMs);
    const result = await db
      .update(schema.agentMissions)
      .set({ status: "running", lastRunAt: new Date(), updatedAt: new Date() })
      .where(
        and(
          eq(schema.agentMissions.id, id),
          or(
            sql`${schema.agentMissions.status} != 'running'`,
            lt(schema.agentMissions.updatedAt, staleBefore),
          ),
        ),
      );
  ...
  async getSignalQueueByUserId(
    userId,
    { status, platform, notebookId, limit = 50 } = {},
  ) {
    const db = await getDatabase();
  ...
  async getSignalQueueItemById(id) {
    ...
    const result = await db.all(
      sql`SELECT * FROM signal_queue WHERE id = ${id} LIMIT 1`,
    );
    return result[0] || null;
  },
  ...
  async updateSignalQueueItem(id, userId, updates) {
    ...
    if (updates.scheduled_for !== undefined) {
      ...
      values.push(
        updates.scheduled_for
          ? Math.floor(new Date(updates.scheduled_for).getTime() / 1000)
          : null,
      );
    }
    if (updates.posted_at !== undefined) {
      ...
      values.push(
        updates.posted_at
          ? Math.floor(new Date(updates.posted_at).getTime() / 1000)
          : null,
      );
    }
    ...
    await db.$client.execute({
      sql: `UPDATE signal_queue SET ${fields.join(", ")} WHERE id = ? AND user_id = ?`,
      args: values,
    });
  },
  ...
  async deleteSignalQueueItem(id, userId) {
    ...
    const result = await db.$client.execute({
      sql: "DELETE FROM signal_queue WHERE id = ? AND user_id = ?",
      args: [id, userId],
    });
