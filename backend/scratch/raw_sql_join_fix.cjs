...
const cleanFunction = `async joinNotebookByCode(userId, joinCode) {
    ...
    // 2. Check if membership already exists - using camelCase keys for Drizzle select
    ...
    // 3. Perform the insertion - using raw SQL to bypass Drizzle mapper confusion
    try {
...
console.log('SUCCESS: joinNotebookByCode has been surgically hardened with raw SQL bypass.');
