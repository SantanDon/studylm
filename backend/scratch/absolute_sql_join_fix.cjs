...
const cleanFunction = `async joinNotebookByCode(userId, joinCode) {
    ...
    // 3. Perform the insertion - using raw SQL to ensure absolute control over column mapping
    try {
      const memberId = "mem-" + Math.random().toString(36).substring(2, 15);
      const now = new Date();
      
      // We manually build the SQL query to ensure "joined_at" is mapped correctly to the integer timestamp
      await db.run(sql\`
        INSERT INTO notebook_members (id, notebook_id, user_id, role, joined_at) 
        VALUES (\${memberId}, \${notebook.id}, \${userId}, 'editor', \${now.getTime()})
      \`);
...
console.log('SUCCESS: joinNotebookByCode has been hardened with absolute SQL mapping.');
