...
// Fixed function using explicit Date object for joinedAt to satisfy Drizzle mapper
const cleanFunction = `async joinNotebookByCode(userId, joinCode) {
    ...
    // Insertion with explicit joinedAt column
    try {
      ...
      console.log('[DB] Joined:', notebook.id);
    } catch (e) {
      if (!e.message.includes('UNIQUE') && !e.message.includes('already exists')) throw e;
    }
...
console.log('SUCCESS: Join restored with explicit joinedAt.');
