...
const ACCESS_VALUE =
  process.env.AUDIOBOOK_ACCESS || `guest_${"audiobook_local"}`;
const FOREIGN_ACCESS = `guest_${"audiobook_foreign"}`;
const POLL_INTERVAL_MS = 800;
const TIMEOUT_MS = 60_000;

const authHeader = (access = ACCESS_VALUE) => ({
  Authorization: `Bearer ${access}`,
});
const jsonHeader = (access = ACCESS_VALUE) => ({
...
function validateWav(buffer, label) {
  if (buffer.length < 44)
    throw new Error(`${label} is too small to be a WAV file`);
  const riff = buffer.toString("ascii", 0, 4);
  const wave = buffer.toString("ascii", 8, 12);
  if (riff !== "RIFF" || wave !== "WAVE")
    throw new Error(`${label} has invalid WAV header: ${riff}/${wave}`);
...
The full audiobook job should concatenate generated chapter audio into one downloadable WAV file. This keeps the production Kokoro path separate from the fast local diagnostic path.`;
  ...
  const tempPath = path.join(
    os.tmpdir(),
    `studypod-audiobook-local-${Date.now()}.txt`,
  );
...
async function fetchJson(pathname, options = {}) {
  ...
  if (!res.ok)
    throw new Error(
      `${pathname} failed (${res.status}): ${JSON.stringify(data)}`,
    );
  return data;
}
...
async function stageHealth() {
  ...
  divider();
  const data = await fetchJson("/api/audiobook/health", {
    headers: authHeader(),
  });
...
async function stageUpload() {
  ...
  try {
    ...
    if (!data.chapters?.length)
      throw new Error("No chapters returned from extraction");
...
async function stageChapterAudio(book) {
  ...
  divider();
  const chapter =
    book.chapters.find((ch) => ch.hasText !== false) || book.chapters[0];
  ...
  const foreignRes = await fetch(`${BASE_URL}${url}`, {
    headers: authHeader(FOREIGN_ACCESS),
  });
  await foreignRes.text();
  if (foreignRes.status !== 404)
    throw new Error(
      `Foreign chapter access returned ${foreignRes.status}, expected 404`,
    );
  pass("Chapter audio is isolated from another guest account");

  const res = await fetch(`${BASE_URL}${url}`, { headers: authHeader() });
  const buffer = Buffer.from(await res.arrayBuffer());
  if (!res.ok)
    throw new Error(
      `Chapter generation failed (${res.status}): ${buffer.toString("utf8")}`,
    );
  const wav = validateWav(buffer, "chapter audio");
  pass(`Chapter: ${chapter.title}`);
  pass(
    `WAV: ${(wav.bytes / 1024).toFixed(1)} KB, ${wav.durationSec.toFixed(1)}s, ${wav.sampleRate}Hz`,
  );
  return { chapter, wav };
}
...
async function stageFullBook(book) {
  ...
  const foreignStatus = await fetch(
    `${BASE_URL}/api/audiobook/job-status/${job.jobId}`,
    { headers: authHeader(FOREIGN_ACCESS) },
  );
  await foreignStatus.text();
  if (foreignStatus.status !== 404)
    throw new Error(
      `Foreign job access returned ${foreignStatus.status}, expected 404`,
    );
  pass("Job status is isolated from another guest account");

  const started = Date.now();
  let lastProgress = -1;
  while (Date.now() - started < TIMEOUT_MS) {
    await sleep(POLL_INTERVAL_MS);
    const status = await fetchJson(`/api/audiobook/job-status/${job.jobId}`, {
      headers: authHeader(),
    });
    ...
    if (status.status === "failed")
      throw new Error(status.error || "Full audiobook job failed");
    if (status.status === "completed") {
      if (!status.url) throw new Error("Completed job missing download URL");
      pass(`Completed: ${status.url}`);
      const res = await fetch(`${BASE_URL}${status.url}`, {
        headers: authHeader(),
      });
      const buffer = Buffer.from(await res.arrayBuffer());
      ...
      pass(
        `Merged WAV: ${(wav.bytes / 1024).toFixed(1)} KB, ${wav.durationSec.toFixed(1)}s`,
      );
      const foreignDownload = await fetch(`${BASE_URL}${status.url}`, {
        headers: authHeader(FOREIGN_ACCESS),
      });
      await foreignDownload.text();
      if (foreignDownload.status !== 404)
        throw new Error(
          `Foreign download returned ${foreignDownload.status}, expected 404`,
        );
      pass(
        "Completed audiobook download is isolated from another guest account",
      );
      return status;
    }
  }
...
async function main() {
  ...
  console.log(
    "║      StudyPodLM Local Audiobook Validation                    ║",
  );
  console.log(
    "╠════════════════════════════════════════════════════════════════╣",
  );
