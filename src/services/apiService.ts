...
/**
 * Refresh the secure browser session once, even when several startup requests
 * discover an expired access cookie at the same time. Session credentials stay
 * in httpOnly cookies and are never exposed to application code.
 */
async function refreshCookieSession(): Promise<boolean> {
  if (!cookieRefreshInFlight) {
    cookieRefreshInFlight = fetch(`${API_BASE_URL}/auth/refresh`, {
      method: "POST",
      credentials: "include",
    })
...
export interface AgentMission {
  ...
  maxNotes: number;
  result?: AgentMissionResult | null;
...
interface AgentMissionRunResponse {
 ...
 * A 401 means authentication is no longer valid. A 403 is an authorization
 * decision for the current action and must not destroy an otherwise valid
 * session.
 */
async function handleDocumentResponse<T>(response: Response): Promise<T> {
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      errorData.error || "Document request failed",
    ) as Error & {
      code?: string;
      currentVersion?: number;
    };
...
export const ApiService = {
  ...
  async resetPassphrase(resetToken: string, newPassphrase: string) {
    ...
    return response.json(); // { success, user }; secure cookies carry the session
  },
  ...
  async getYouTubeUsage(token: string): Promise<{
    limit: number;
    used: number;
    remaining: number;
    resetAt: string;
  ...
  async recordYouTubeExtractionSuccess(
    ...
    const response = await fetch(
      `${API_BASE_URL}/user/usage/youtube/record-success`,
      {
        method: "POST",
        headers,
  ...
  async fetchSources(notebookId: string, token: string) {
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/sources`,
      {
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
      },
  ...
  async updateSource(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/sources/${sourceId}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async updateNote(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/notes/${noteId}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async getPendingAgentUploads(
    notebookId: string,
    token: string,
    signal?: AbortSignal,
  ) {
    const response = await fetch(
      `${API_BASE_URL}/agent/pending-uploads?notebookId=${notebookId}`,
      {
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
        signal,
  ...
  async downloadAgentUpload(uploadId: string, token: string): Promise<Blob> {
    ...
    if (!response.ok)
      throw new Error("Failed to download pending agent upload");
  ...
  async fetchSignalQueue(
    ...
    const response = await fetch(
      `${API_BASE_URL}/signal-queue?${query.toString()}`,
      {
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
      },
  ...
  async importTweets(
    notebookId: string,
    token: string,
    data: { urls?: string[]; fileContent?: string },
  ) {
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/sources/tweets`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async fetchAgentMissions(
    notebookId: string,
    token: string,
  ): Promise<AgentMissionListResponse> {
    const params = new URLSearchParams({ notebookId });
    const response = await fetch(
      `${API_BASE_URL}/agent/missions?${params.toString()}`,
      {
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
      },
  ...
  async createAgentMission(
    ...
    const response = await fetch(`${API_BASE_URL}/agent/missions`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
      body: JSON.stringify({ notebookId, goal, maxNotes }),
      credentials: "include",
    });
    return handleResponse(response);
  },

  async runAgentMission(
    missionId: string,
    token: string,
  ): Promise<AgentMissionRunResponse> {
    const response = await fetch(
      `${API_BASE_URL}/agent/missions/${missionId}/run`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
  ...
  async deleteAgentMission(missionId: string, token: string): Promise<void> {
    const response = await fetch(
      `${API_BASE_URL}/agent/missions/${missionId}`,
      {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
  ...
  async fetchResearchGoals(
    notebookId: string,
    token: string,
    opts: { includeArchived?: boolean; parentGoalId?: string | null } = {},
  ) {
  ...
  async createResearchGoal(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/research-goals`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async patchResearchGoal(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/research-goals/${goalId}`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async linkTaskToGoal(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/research-goals/${goalId}/tasks/${taskId}`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
  ...
  async linkArtifactToGoal(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/research-goals/${goalId}/artifacts`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async fetchSuggestedGoals(
    notebookId: string,
    sourceId: string,
    token: string,
  ) {
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/sources/${sourceId}/suggested-goals`,
      {
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
      },
  ...
  async acceptSuggestedGoal(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/sources/${sourceId}/suggested-goals/${suggestionId}/accept`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
  ...
  async researchNotebook(
    notebookId: string,
    options: { query?: string; depth?: "quick" | "deep" },
    token: string,
  ) {
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/research`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async fetchDocuments(
    ...
    const data = await handleDocumentResponse<{
  ...
  async fetchDocument(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/documents/${documentId}`,
      {
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
      },
    );
    const data = await handleDocumentResponse<{ document: DocumentArtifact }>(
      response,
    );
  ...
  async createDocumentFromSource(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/documents/from-source`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async fetchDocumentVersions(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/documents/${documentId}/versions`,
      {
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
      },
    );
    const data = await handleDocumentResponse<{ versions: DocumentVersion[] }>(
      response,
    );
  ...
  async restoreDocumentVersion(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/documents/${documentId}/versions/${version}/restore`,
      {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
  ...
  async proposeDocumentRevision(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/documents/${documentId}/revisions/propose`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async applyDocumentRevision(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/documents/${documentId}/revisions/apply`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async exportDocument(
    ...
    const response = await fetch(
      `${API_BASE_URL}/notebooks/${notebookId}/documents/${documentId}/export?format=${format}`,
      {
        headers: { Authorization: `Bearer ${token}` },
        credentials: "include",
      },
    ...
    const filename = encoded
      ...
      : simple || `StudyPod-Document.${format}`;
    return {
      blob: await response.blob(),
      filename,
      version:
        Number(response.headers.get("x-studypod-document-version")) || null,
    };
  },
};
